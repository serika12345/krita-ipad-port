...
usage() {
    cat >&2 <<'EOF'
usage: build-krita-incremental.sh <command> [arguments]
...
script_path="$script_dir/build-krita-incremental.sh"

# Record a source-independent environment once per Nix/configuration revision.
...
if [[ "${KRITA_IOS_INCREMENTAL_ENV:-0}" != "1" ]]; then
    ...
    if [[ ! -e "$environment_profile" ]] \
        ...
        nix develop .#krita-ios-incremental \
...
if [[ -z "${cmakeFlags:-}" ]]; then
    echo "error: the krita-ios-app development environment did not provide cmakeFlags" >&2
    exit 1
fi
