...
The first M4 increments link the KRA and PNG import/export plugins and the
default Pixel Brush paint-op statically into the unsigned iPadOS application.
This validates the common conversion, registration, metadata discovery, and
dead-stripping path before expanding the iOS plugin profile to tool and Docker
plugins.
...
The executable contains all five complete static plugin paths. The relevant
...
The KRA, PNG, and paint-op service metadata is present in the linked image. The
rebuilt app is Mach-O arm64 for platform IOS, minimum iOS 17.0, built against
SDK 26.5. Its Info.plist passes `plutil -lint`, and `otool -L` shows only iOS
system libraries and frameworks. The unsigned bundle is approximately 83 MiB.
...
and remain part of M5. M4 is not complete: basic tools, Layer Docker, and the
user-facing iOS feature profile still need to be added and validated. Each older
plugin factory macro must be changed to
